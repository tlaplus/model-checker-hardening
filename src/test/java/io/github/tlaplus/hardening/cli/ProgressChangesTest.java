...
import static io.github.tlaplus.hardening.cli.RunValue.Direction.*;
...
class ProgressChangesTest {
    ...
    void comparesUnroundedValuesInBothDirectionsAndRenewsExpiry() {
        ...
        assertEquals(Map.of(count, UP, queue, DOWN), changes.highlighted(1_000_000_000L));
        changes.observe(next, 1_500_000_000L);
        assertEquals(Map.of(count, UP, queue, DOWN), changes.highlighted(1_999_999_999L));
        assertTrue(changes.highlighted(2_000_000_000L).isEmpty());

        changes.observe(first, 2_000_000_001L);
        assertEquals(Map.of(count, DOWN, queue, UP), changes.highlighted(2_000_000_001L));
        changes.observe(next, 2_500_000_001L);
        assertEquals(Map.of(count, UP, queue, DOWN), changes.highlighted(3_000_000_001L));
        assertTrue(changes.highlighted(3_500_000_001L).isEmpty());
    }

    @Test
    void tracksRichnessGenerationAndStatusDirectionsButNotElapsedClocks() {
        var changes = new ProgressChanges();
        ...
        assertEquals(Map.of(RunMetric.Field.MIN_RICHNESS, UP, RunMetric.Field.GENERATION, UP,
                RunMetric.State.STATUS, CHANGED), changes.highlighted(10));
        values.put(RunMetric.Field.MIN_RICHNESS, RunValue.Richness.of(0, 0));
        changes.observe(values, 20);
        assertEquals(DOWN, changes.highlighted(20).get(RunMetric.Field.MIN_RICHNESS));
    }
}
