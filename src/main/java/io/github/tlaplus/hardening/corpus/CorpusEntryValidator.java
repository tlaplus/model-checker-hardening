 ...
 * decisions from here. The whole {@link CorpusEnvelope} is supplied because the kind and the
 * admission metadata are part of that decision. An implementation reports an input it cannot use as a {@link CorpusException} naming
 * the entry.
 */
@FunctionalInterface
public interface CorpusEntryValidator {
     ...
     */
    void validate(Path entry, CorpusEnvelope envelope) throws CorpusException;

    /** Accepts every input, for operations that read an entry rather than validate it. */
    CorpusEntryValidator NONE = (entry, envelope) -> {};
}
